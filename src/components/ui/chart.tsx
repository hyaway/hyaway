...
const XAxis = ({
  ...
  return (
    <XAxisPrimitive
      className={twMerge(
        "text-muted-fg **:[text]:fill-muted-fg text-xs",
        className,
      )}
...
const ChartTooltipContent = <TValue extends ValueType, TName extends NameType>({
  ...
  return (
    <div
      ref={ref}
      className={twMerge(
        "bg-overlay/70 text-overlay-fg grid min-w-[12rem] items-start rounded-lg p-3 py-2 text-xs ring ring-current/10 backdrop-blur-lg",
        className,
      )}
    >
      {!hideLabel && (
        <>
          ...
          {labelSeparator && (
            <span
              aria-hidden
              className="bg-bg/10 mt-2 mb-3 block h-px w-full"
            />
          )}
        </>
      )}
