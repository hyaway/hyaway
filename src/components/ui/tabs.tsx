      ...
      {(values) => (
        <>
          ...
          <SelectionIndicator
            ...
            className={twMerge(
              "bg-primary-subtle-fg absolute transition-[translate,width,height] duration-200",
              orientation === "horizontal"
...
const TabPanel = ({ className, ref, ...props }: TabPanelProps) => {
  return (
    <TabPanelPrimitive
      ...
      data-slot="tab-panel"
      className={cx(
        "text-fg flex-1 text-sm/6 focus-visible:outline-hidden",
        className,
      )}
    />
  );
...
export { Tabs, TabList, Tab, TabLink, TabPanel };
