...
const ListBox = <T extends object>({
  ...
  <ListBoxPrimitive
    {...props}
    className={cx(
      "bg-bg grid max-h-96 w-full min-w-56 scroll-py-1 grid-cols-[auto_1fr] flex-col gap-y-1 overflow-y-auto overscroll-contain rounded-xl border p-1 outline-hidden [scrollbar-width:thin] has-data-[slot=drag-icon]:grid-cols-[auto_auto_1fr] [&::-webkit-scrollbar]:size-0.5 *:[[role='group']+[role=group]]:mt-4 *:[[role='group']+[role=separator]]:mt-1",
      className,
    )}
  />
);
        ...
        return (
          <>
            {allowsDragging && (
              <svg
                data-slot="drag-icon"
                className="text-muted-fg mr-2 size-5 h-[1lh] sm:w-4"
