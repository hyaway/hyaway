...
type RatingsOverlaySize = "sm" | "md" | "lg";
...
function RatingBadge({ service, value, size }: RatingBadgeProps) {
  const iconClass = cn(
    size === "sm" && "size-2.5",
    size === "md" && "size-3.5",
    size === "lg" && "size-4",
  ...
  const badgeClass = cn(
    "bg-card/80 flex items-center rounded-xs font-extrabold tabular-nums dark:font-semibold",
